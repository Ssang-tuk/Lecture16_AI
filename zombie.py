...
class Zombie:
    ...
    def draw(self):
        ...
        draw_circle(self.x, self.y, int(7.0 * PIXEL_PER_METER), 1, 1, 1)
    ...
    def build_behavior_tree(self):
        ...
        c_have_more = Condition('좀비가 공 >= 소년?', self.have_more_balls)

        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        a4 = Action('소년한테 접근', self.move_to_boy)
        root = chase_boy_if_nearby = Sequence('소년을 추적', c_have_more, a4)

        a5 = Action('순찰 위치 가져오기', self.get_patrol_location)
        # root = patrol = Sequence('순찰', a5, a2)

        # root = chase_if_boy_near_or_patrol = Selector('추적 또는 순찰', chase_boy_if_nearby, patrol)
        root = Selector('행동', Sequence('근접 검사 후 행동', c1, chase_boy_if_nearby), wander)
